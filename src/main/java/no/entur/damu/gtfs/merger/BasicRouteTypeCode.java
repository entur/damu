...
public enum BasicRouteTypeCode {
  ...
  public static int convertRouteType(int extendedType) {
    ...
    } else if (extendedType >= 300 && extendedType < 500) { //Suburban Railway Service and Urban Railway service
    ...
    if (extendedType >= 1500 && extendedType < 1600) {
      LOGGER.info(
        "Taxi extended route type {} is not supported in basic GTFS route type. Mapping to bus as default. ",
        extendedType
      );
    } else {
      LOGGER.warn(
      ...
      throw new IllegalArgumentException(
        "Attempted to map unsupported extended route type to basic GTFS route type."
      );
    }
    return BasicRouteTypeCode.BUS.code;
  }
