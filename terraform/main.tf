...
resource "google_service_account" "damu_service_account" {
...
resource "google_storage_bucket_iam_member" "storage_marduk_bucket_iam_member" {
  ...
  member = "serviceAccount:${google_service_account.damu_service_account.email}"
}
...
resource "google_service_account_key" "damu_service_account_key" {
  service_account_id = google_service_account.damu_service_account.name
}

# Add SA key to to k8s
resource "kubernetes_secret" "damu_service_account_credentials" {
  metadata {
    name = "${var.labels.team}-${var.labels.app}-sa-key"
    namespace = var.kube_namespace
  }
  data = {
    "credentials.json" = base64decode(google_service_account_key.damu_service_account_key.private_key)
  }
}
